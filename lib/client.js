...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const css = '.dshm_badge{width:100%;height:49px;color:var(--dsw-alias-label-primary);cursor:pointer;background:0 0;border:none;border-radius:12px;align-items:center;gap:8px;padding:0 8px 0 6px;font-family:inherit;font-size:14px;display:inline-flex;overflow:hidden}'
      ...
      // 安全事件横幅/角标(2026-08-20 审计缓解:配对完成/吊销/Web 密码变更显性化)
    ...
    function MobileAccessAction({ wide }) {
      ...
      // 中转服务器注册表(/api/gateways):列表 + 配对目标选择器 + 卡片编辑器。
      const [gatewaysInfo, setGatewaysInfo] = R.useState(null)
      const [selectedGw, setSelectedGw] = R.useState('')
      const [gwDraft, setGwDraft] = R.useState(null)
      const [gwBusy, setGwBusy] = R.useState(false)
      const [gwMsg, setGwMsg] = R.useState(null)
      ...
      // 常态轮询:gateways(本地聚合,含隧道状态)+ tokens(经管理通道)。
      R.useEffect(() => {
        let alive = true
        const tick = async () => {
          if (document.hidden) return
        const [g, k, l, w, s] = await Promise.allSettled([
          jget(PAIR_API + '/gateways'), jget(PAIR_API + '/tokens'), jget(PAIR_API + '/label'),
          jget(PAIR_API + '/web-password'), jget(PAIR_API + '/security-log'),
        ])
        if (!alive) return
        if (g.status === 'fulfilled') {
          setGatewaysInfo(g.value)
          // 选择器默认落第一个管理通道就绪的条目;已选条目消失时同样回落。
          const list = g.value.gateways ?? []
          setSelectedGw((prev) => (list.some((x) => x.id === prev)
            ? prev
            : ((list.find((x) => x.adminReady) ?? list[0]) ?? { id: '' }).id))
        }
        ...
        if (s.status === 'fulfilled') setSec(s.value)
        }
        tick()
      ...
      const startScan = async () => {
        ...
        try { setPair(await jpost(PAIR_API + '/start', { gatewayId: selectedGw })) }
      ...
      const startManual = async () => {
        setBusy(true); setActErr(null)
        try {
          const s = await jpost(PAIR_API + '/claim', { gatewayId: selectedGw, code: manual })
      ...
      // 服务器卡片编辑器:upsert 保存(密钥留空 = 保持不变,由服务端对
      // undefined 语义保留);删除需确认。
      const saveGateway = async () => {
        setGwBusy(true); setGwMsg(null)
        try {
          const d = gwDraft
          const entry = {
            id: d.id, type: d.type, name: d.name, pairUrl: d.pairUrl,
            ...(d.type === 'rust'
              ? { target: d.target, adminPort: d.adminPort, remotePort: d.remotePort, sockDir: d.sockDir, adminUrl: d.adminUrl }
              : { gateway: d.gateway, cfTunnelId: d.cfTunnelId, cfHostname: d.cfHostname }),
          }
          // 密钥仅在用户输入了值时携带(undefined = 服务端保留现值)。
          if (d.adminKey !== '') entry.adminKey = d.adminKey
          if (d.tenantKey !== '') entry.tenantKey = d.tenantKey
          const r = await jpost(PAIR_API + '/gateways', { entry })
          if (r && r.ok) {
            setGwDraft(null)
            setGwMsg(r.restartTunnel ? '已保存;隧道参数改动需重启 dsh web 后生效' : '已保存')
            setGatewaysInfo(await jget(PAIR_API + '/gateways'))
          } else setGwMsg('保存失败:' + String(r && r.error ? r.error : '未知错误'))
        } catch (e) { setGwMsg('保存失败:' + String(e.message)) }
        setGwBusy(false)
      }
      const removeGateway = async (id, name) => {
        if (!window.confirm('删除中转服务器「' + String(name || id) + '」?已配对令牌不受影响,但新配对不再经过它。')) return
        setGwBusy(true); setGwMsg(null)
        try {
          const r = await jpost(PAIR_API + '/gateways', { remove: true, id })
          if (r && r.ok) setGatewaysInfo(await jget(PAIR_API + '/gateways'))
          else setGwMsg('删除失败:' + String(r && r.error ? r.error : '未知错误'))
        } catch (e) { setGwMsg('删除失败:' + String(e.message)) }
        setGwBusy(false)
      }
      ...
      const ackSecurity = async () => {
        try { await jpost(PAIR_API + '/security-log/ack') } catch (e) {}
        try { setSec(await jget(PAIR_API + '/security-log')) } catch (e) {}
      }
      ...
      const pairArea = (() => {
        if (pair === null) {
          // 扫码/粘贴邀请模式:QR 编码所选服务器的配对入口 —— 手机发起的
          // 网关由二维码决定,与面板选择器一致(锚定是有意设计:防两边各打
          // 一个网关的错位)。
          const selGw = (gatewaysInfo === null ? [] : (gatewaysInfo.gateways ?? [])).find((x) => x.id === selectedGw)
          const anchorHost = (() => {
            try { return new URL(selGw && selGw.pairUrl ? selGw.pairUrl : '').host } catch { return '' }
          })()
          return h('div', { key: 'idle' }, [
            h('div', { key: 'row', className: 'dshm_row', style: { marginTop: '4px' } }, [
              h('button', { key: 'scan', type: 'button', className: 'dshm_btn', disabled: busy || (selGw !== undefined && !selGw.adminReady), onClick: startScan }, '配对手机(扫码)'),
            ]),
            h('div', { key: 'hint', className: 'dshm_dim', style: { marginTop: '6px' } },
              '二维码编码所选服务器「' + esc(selGw ? (selGw.name || selGw.id) : '—') + '」的配对入口' + (anchorHost ? '(' + anchorHost + ')' : '') +
              ' —— 手机扫码后连该服务器。App 相机扫码;或系统相机扫 → 落地页「复制」→ App 粘贴(同样以邀请里的服务器为准)'),
          ])
        }
        ...
        h('td', { key: 'p' }, [
          t.viaType === 'cf'
            ...
            : t.viaType === 'rust'
              ...
              : null,
          ' ' + esc(t.via || (t.upstream_port == null ? '-' : String(t.upstream_port))),
        ]),
      ...
      // ── 中转服务器 ── 注册表卡片:两个状态点独立亮灯(「管理通道」决定
      // 能否配对;「隧道」决定配对后手机能否真连上);密钥在编辑器里填,
      // 留空保存 = 保持现值。隧道参数改动需重启 dsh web(服务端返回提示)。
      const gwList = gatewaysInfo === null ? null : (gatewaysInfo.gateways ?? [])
      const gwCard = (g) => h('div', { key: g.id, style: { marginTop: '8px' } }, [
        h('div', { key: 'row', className: 'dshm_row' }, [
          h('span', { key: 'a', className: g.adminReady ? 'dshm_dot dshm_on' : 'dshm_dot', title: '管理通道(配对/设备管理)' }),
          h('span', { key: 'tg', className: g.type === 'cf' ? 'dshm_tag dshm_tagCf' : 'dshm_tag dshm_tagRust' }, g.type === 'cf' ? 'CF' : 'Rust'),
          h('span', { key: 'n' }, esc(g.name || g.id)),
          h('span', { key: 't', className: 'dshm_dim' },
            g.tunnel === null ? '无隧道(仅管理面)'
              : (g.tunnel.up ? '隧道✓' : '隧道重连中(第 ' + String(g.tunnel.attempts) + ' 次)')),
          selectedGw === g.id ? h('span', { key: 'sel', className: 'dshm_tag dshm_tagOn' }, '配对目标') : null,
          h('button', { key: 'e', type: 'button', className: 'dshm_btn', onClick: () => { setGwDraft({ ...g, adminKey: '', tenantKey: '' }); setGwMsg(null) } }, '编辑'),
          gatewaysInfo !== null && gatewaysInfo.editable === false ? null
            : h('button', { key: 'd', type: 'button', className: 'dshm_btnWarn', disabled: gwBusy, onClick: () => removeGateway(g.id, g.name) }, '删除'),
        ]),
        h('div', { key: 'pu', className: 'dshm_dim' },
          '配对入口 ' + esc(g.pairUrl || '(未填)')
          + (g.type === 'rust'
              ? ' · 落点 :' + String(g.remotePort ?? '-') + (g.sockDir ? '(UDS ' + esc(g.sockDir) + ')' : '')
                + (g.adminUrl ? ' · 多租户 ' + esc(g.adminUrl) : '') + (g.target ? ' · ssh ' + esc(g.target) : '')
              : (g.cfTunnelId ? ' · cloudflared ' + esc(g.cfHostname) : ''))),
      ])

      const gwField = (key, label, props) => h('div', { key: 'f-' + key, style: { marginTop: '6px' } }, [
        h('div', { key: 'l', className: 'dshm_dim' }, label),
        h('input', Object.assign({ key: 'in', className: 'dshm_input', style: { letterSpacing: 0 }, autoComplete: 'off', spellCheck: false }, props)),
      ])
      const gwEditor = gwDraft === null ? null : h('div', { key: 'gwEditor', style: { border: '1px solid var(--dsw-alias-border-l2)', borderRadius: '10px', padding: '10px', marginTop: '8px' } }, [
        h('div', { key: 't', className: 'dshm_dim' }, (gwDraft.id ? '编辑 ' : '添加 ') + (gwDraft.type === 'cf' ? 'CF' : 'Rust') + ' 服务器'),
        gwField('name', '名称', { value: gwDraft.name, placeholder: '如 yltech.store', onChange: (e) => setGwDraft({ ...gwDraft, name: e.target.value }) }),
        gwField('pairUrl', '配对入口 URL(二维码编码它,决定手机锚定哪台)', { value: gwDraft.pairUrl, placeholder: 'https://dsh.example.com/pair', onChange: (e) => setGwDraft({ ...gwDraft, pairUrl: e.target.value }) }),
        ...(gwDraft.type === 'rust' ? [
          gwField('target', 'ssh target(运营者管理通道 + ssh -R 隧道;多租户形态可留空)', { value: gwDraft.target, placeholder: 'user@server', onChange: (e) => setGwDraft({ ...gwDraft, target: e.target.value }) }),
          gwField('remotePort', '隧道远端端口(同服务器唯一,见落点占用提示)', { value: String(gwDraft.remotePort ?? ''), placeholder: '13101', onChange: (e) => setGwDraft({ ...gwDraft, remotePort: e.target.value }) }),
          gwField('sockDir', 'UDS 目录(可空 = TCP 落点)', { value: gwDraft.sockDir, placeholder: '/run/dsh-gateway', onChange: (e) => setGwDraft({ ...gwDraft, sockDir: e.target.value }) }),
          gwField('adminUrl', 'adminUrl(多租户 HTTPS 管理面;配租户密钥后免 ssh)', { value: gwDraft.adminUrl, placeholder: 'https://gw.example.com', onChange: (e) => setGwDraft({ ...gwDraft, adminUrl: e.target.value }) }),
          gwField('tenantKey', '租户密钥' + (gwDraft.tenantKeyConfigured ? '(已配置,留空保存 = 保持不变)' : '(运营者经 /admin/tenants 签发)'), { type: 'password', value: gwDraft.tenantKey, onChange: (e) => setGwDraft({ ...gwDraft, tenantKey: e.target.value }) }),
        ] : [
          gwField('gateway', 'Worker 网关地址', { value: gwDraft.gateway, placeholder: 'https://worker.example.com', onChange: (e) => setGwDraft({ ...gwDraft, gateway: e.target.value }) }),
          gwField('adminKey', 'ADMIN_KEY' + (gwDraft.adminKeyConfigured ? '(已配置,留空保存 = 保持不变)' : '(部署 Worker 时设置)'), { type: 'password', value: gwDraft.adminKey, onChange: (e) => setGwDraft({ ...gwDraft, adminKey: e.target.value }) }),
          gwField('cfTunnelId', 'cloudflared Tunnel UUID(可空 = 不拉隧道)', { value: gwDraft.cfTunnelId, placeholder: 'ce73ed0a-…', onChange: (e) => setGwDraft({ ...gwDraft, cfTunnelId: e.target.value }) }),
          gwField('cfHostname', '隧道公网主机名', { value: gwDraft.cfHostname, placeholder: 'dsh-xx.example.com', onChange: (e) => setGwDraft({ ...gwDraft, cfHostname: e.target.value }) }),
        ]),
        h('div', { key: 'act', className: 'dshm_row', style: { marginTop: '8px' } }, [
          h('button', { key: 'go', type: 'button', className: 'dshm_btn', disabled: gwBusy, onClick: saveGateway }, gwBusy ? '保存中…' : '保存'),
          h('button', { key: 'x', type: 'button', className: 'dshm_btnWarn', onClick: () => setGwDraft(null) }, '取消'),
          gwMsg === null ? null : h('span', { key: 'm', className: gwMsg.startsWith('已') ? 'dshm_ok' : 'dshm_bad' }, esc(gwMsg)),
        ]),
      ])

      // ── 安全事件横幅(未确认事件存在时置顶显示;数据源 /api/security-log)──
      const SEC_LABEL = { paired: '新设备完成配对', revoked: '设备令牌被吊销', 'web-password': 'Web 面密码变更' }
      ...
      const secBanner = secEvents.length === 0 ? null : h('div', { key: 'secBanner', className: 'dshm_sec' }, [
        h('div', { key: 't', className: 'dshm_secTitle' },
          '⚠ 安全事件未确认(' + String(secEvents.length) + ')— 若非你本人操作,请立即吊销设备 / 清除 Web 密码'),
      ...
      const dialog = !open ? null : h('div', {
        ...
        h('div', { key: 'body', className: 'dshm_body' }, [
          secBanner,
          h('div', { key: 'h-gw', className: 'dshm_h2' }, '中转服务器'),
          h('div', { key: 'gw' }, gwList === null
            ? h('div', { className: 'dshm_dim' }, '读取中…')
            : [gwList.length === 0 ? h('div', { key: 'empty', className: 'dshm_dim' }, '(未配置 —— 添加一台网关后才能配对)') : null,
               ...gwList.map(gwCard)].filter(Boolean)),
          gatewaysInfo !== null && gatewaysInfo.editable !== false && gwDraft === null
            ? h('div', { key: 'gwAdd', className: 'dshm_row', style: { marginTop: '8px' } }, [
                h('button', { key: 'r', type: 'button', className: 'dshm_btn', onClick: () => setGwDraft({ id: '', type: 'rust', name: '', pairUrl: '', target: '', adminPort: 8103, remotePort: '', sockDir: '', adminUrl: '', tenantKey: '', gateway: '', adminKey: '', cfTunnelId: '', cfHostname: '' }) }, '＋ Rust 服务器'),
                h('button', { key: 'c', type: 'button', className: 'dshm_btn', onClick: () => setGwDraft({ id: '', type: 'cf', name: '', pairUrl: '', target: '', adminPort: 8103, remotePort: '', sockDir: '', adminUrl: '', tenantKey: '', gateway: '', adminKey: '', cfTunnelId: '', cfHostname: '' }) }, '＋ CF 服务器'),
              ])
            : null,
          gwEditor,
          ...
          webInfo !== null && webInfo.error === undefined && h('div', { key: 'web' }, [
            h('div', { key: 'st', className: 'dshm_dim' }, webInfo.enabled
              ? '已启用(版本 v' + String(webInfo.version) + ')— 浏览器打开网关 web 域名,选择「' + esc(webInfo.label || '本机') + '」输此密码登录'
              : '未启用 —— 设置密码后,浏览器打开网关 web 域名会出现本机宿主选项,输此密码登录'),
          ...
          // 服务器选择器(多台才出现;单台时卡片已标「配对目标」):扫码 QR
          // 与手输应约都定向到所选服务器 —— 手机等在哪台,这里选哪台。
          gwList !== null && gwList.length > 1 ? h('div', { key: 'gwSel', className: 'dshm_row', style: { marginTop: '4px' } },
            gwList.map((g) => h('button', {
              key: g.id, type: 'button', className: 'dshm_btn',
              style: g.id === selectedGw ? { outline: '2px solid var(--dsw-alias-interactive-accent)' } : {},
              disabled: !g.adminReady,
              title: g.adminReady ? '扫码与手输都定向到这台' : '管理通道未就绪(在上方卡片补齐配置/密钥)',
              onClick: () => setSelectedGw(g.id),
            }, (g.adminReady ? '' : '⚠ ') + (g.name || g.id)))
          ) : null,
          pairArea,
          pair === null ? h('div', { key: 'manual', style: { marginTop: '8px' } }, [
            ...
            h('div', { key: 'hint', className: 'dshm_dim', style: { marginTop: '4px' } },
              '手输模式:码只存在于手机选定的那一台服务器(App 生成码时会显示服务器名)—— 上面的选择器选中同一台后再应约;选错会报「没有这个码」'),
          ]) : null,
