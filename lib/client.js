...
//
// p2p-only 重构:无隧道区/中转服务器注册表/管理密钥/Web 密码区,新增
  ...
  factory: (require) => {
    ...
    function MobileAccessAction({ wide }) {
      ...
      const online = R.useMemo(
        () => (p2p && p2p.signal ? p2p.signal.sessions.filter((s) => s.open).length : 0),
        [p2p],
      )
      ...
      R.useEffect(() => {
        ...
        const tick = async () => {
          ...
          const [k, l, s, c] = await Promise.allSettled([
            jget(API + '/tokens'), jget(API + '/label'), jget(API + '/state'),
            jget(API + '/security-log'),
          ])
          if (!alive) return
      ...
      const rowOf = (t) => h('tr', { key: t.jti }, [
        ...
        h('td', { key: 'p' }, t.link === 'p2p'
          ...
          : t.link === 'relay'
            ? h('span', { key: 'rl', className: 'dshm_tagRelay', title: '经网关服务器中转(旧链路/打洞失败回退)' }, '中转')
      ...
      const dialog = !open ? null : h('div', {
        ...
        h('div', { key: 'body', className: 'dshm_body' }, [
          secBanner,
          h('div', { key: 'h-p2p', className: 'dshm_h2' }, 'P2P 状态'),
          p2pArea,
