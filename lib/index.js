...
function createSignalService({ getConfig, emit }) {
  ...
  const attachStream = (sess, sid, dc, localPort) => {
    ...
    sock.on('data', (chunk) => {
      ...
      // 泵体必须收口:pc.close() 到 dc.onClosed 清理 streams 之间,native 通道
      // 可能已关而 TCP data 仍到达,node-datachannel 对已关通道 sendMessageBinary
      // 同步 throw —— 不兜就是 uncaught exception 崩掉整个 dsh 宿主(前次审计
      // 「泵崩宿主」的残留点,紧邻 close 钩子均有 try{} 即证遗漏非取舍)。
      // 兜底 = 断 TCP;close 钩子照常摘流(streams.delete)并发 CLOSE 帧。
