// dsh-mobile — Mac 侧移动接入(dsh 插件,P2P-only 重构版)。
//
// 架构(p2p-only 分支):网关(yltech.store)只做配对 + WebRTC 信令,
// **不再中转任何业务流量** —— ssh -R 反向隧道 / cloudflared / CF Worker
// 网关形态全部移除。手机与 Mac 经 WebRTC DataChannel(DTLS 加密)直连;
// DataChannel 上跑虚拟字节流(vstream,协议见 PROTOCOL.md),Mac 侧落地回
// 127.0.0.1:<dsh web 端口>。手机 App 侧同样起本地回环代理 —— 两端的
// HTTP/WebSocket 栈零改动。
//
//   手机 App ──wss(仅信令)──→ yltech.store 网关 ←─wss(仅信令)─ 本插件
//      └──────── WebRTC DataChannel(直连,业务流量不经服务器)────────┘
//
// 保留的部分:扫码/手输配对(设备令牌仍由网关签发吊销,令牌绑定的端口
// 现在只是信令路由键)、机器名、设备管理 dialog、安全事件显性化。
//
// 信任根不变:管理面仅网关服务器 127.0.0.1 可达(经 ssh);host ticket
// 由管理面签发(TTL 900s,本插件每 5min 刷新)。
//
// 配置(cordis.patch.yml 的 dsh-mobile 行 config;DSH_MOBILE_* 可覆盖):
//   target(ssh 别名) / adminPort / remotePort(宿主标识,13100-13199,
//   仅作信令路由键,不再有隧道)/ publicUrl(扫码落地页)/ label /
//   iceServers(JSON 数组字符串,缺省公共 STUN)
...
// ── ssh 管理通道(BatchMode + ControlMaster 复用 + 沙箱降级)──────────────
...
// ── 配对会话(语义不变;claim 的 port 现在只是信令路由键)──────────────────
const PAIRING_WINDOW_MS = 5.5 * 60 * 1000

function createPairService({ getConfig, getLabel, onEvent }) {
  // 管理面经 ssh(信任根 = ssh key);凭证不出服务器。
  const AH = "-H \"authorization: Bearer $(sed -n 's/^DSH_GATEWAY_ADMIN_TOKEN=//p' /etc/dsh-gateway.env)\" "

  const admin = (path, method, payload) => {
    const { target, adminPort } = getConfig()
    if (!target) return Promise.resolve(null)
    const base = 'http://127.0.0.1:' + String(adminPort ?? 8103)
    const cli = payload !== undefined
      ? "printf %s '" + Buffer.from(JSON.stringify(payload)).toString('base64')
        + "' | base64 -d | curl -s " + AH + "--max-time 8 -X POST " + base + path
        + " -H 'content-type: application/json' --data-binary @-"
      : "curl -s " + AH + "--max-time 8 '" + base + path + "'"
    return sshCurl(target, cli).then((out) => tryJson(out))
  }
  ...
  const unreachable = () => {
    const { target, adminPort } = getConfig()
    return target
      ? 'ssh ' + String(target) + ' 管理口 ' + String(adminPort ?? 8103) + ' 不可达'
      : '未配置 target(ssh 别名)'
  }
    ...
    while (myGen === generation && session !== null) {
      ...
      if (session.state === 'waiting') {
        const j = await admin('/admin/pair/claim', 'POST', {
          ...
          host_label: session.label, port: getConfig().remotePort,
        })
  ...
  return {
    ...
    startManual: async (codeRaw) => {
      ...
      const j = await admin('/admin/pair/claim', 'POST', {
        code, host_code: session.hostCode, host_label: session.label, port: config.remotePort,
      })
    ...
    tokens: async () => {
      ...
      // 只展示绑定本机端口的令牌(多宿主网关;按信令路由键归属过滤)。
      return j.filter((t) => Number(t.upstream_port) === Number(getConfig().remotePort))
    },
    revoke: async (jti) => {
...
function createSignalService({ getConfig, emit }) {
  ...
  const signalUrl = () => {
    const u = new URL(sanitizePairUrl(getConfig().publicUrl))
  ...
  // 不限频会持续刷屏(网关旧版缺 /admin/signal/ticket 实证);
  // 刷新成功即重置,下次真故障第一时间可见。
  let ticketWarnAt = 0
  const TICKET_WARN_THROTTLE_MS = 5 * 60 * 1000
  const refreshTicket = async () => {
    if (stopped) return
    const { target, adminPort, remotePort } = getConfig()
    if (!target) return
    const base = 'http://127.0.0.1:' + String(adminPort ?? 8103)
    const AH = "-H \"authorization: Bearer $(sed -n 's/^DSH_GATEWAY_ADMIN_TOKEN=//p' /etc/dsh-gateway.env)\" "
    const payload = JSON.stringify({ port: remotePort })
    const cli = "printf %s '" + Buffer.from(payload).toString('base64')
      + "' | base64 -d | curl -s " + AH + "--max-time 8 -X POST " + base + '/admin/signal/ticket'
      + " -H 'content-type: application/json' --data-binary @-"
    const j = tryJson(await sshCurl(target, cli))
    if (j !== null && typeof j.ticket === 'string' && j.ticket.length > 0) {
      ticket = j.ticket
      ticketExp = Number(j.expires_at) * 1000
      ticketWarnAt = 0
    } else if (ticketExp < Date.now() + 60_000 && Date.now() - ticketWarnAt > TICKET_WARN_THROTTLE_MS) {
      ticketWarnAt = Date.now()
      emit('warning', 'host ticket 刷新失败(ssh ' + String(target) + ');信令通道将无法重连')
    }
  }
...
// ── loopback 判定 + 同源门(管理 API 三重门,与旧版一致)──────────────────
...
export function apply(ctx, config = {}) {
  ...
  // 键名与旧(中转)版一致:target/adminPort/remotePort/publicUrl —— 存量
  // 部署改插件不改配置即可迁移;remotePort 语义变化:不再是隧道口,只是
  // 信令路由键(网关按它把手机连到本宿主)。
  const runtime = {
    ...
    publicUrl: sanitizePairUrl(process.env.DSH_MOBILE_PUBLIC_URL ?? config?.publicUrl ?? ''),
    iceServers: (() => {
  ...
  if (!runtime.target) {
    throw new Error('dsh-mobile: no target configured (ssh 别名,指向网关服务器)')
  }
  if (!isValidPairUrl(runtime.publicUrl)) {
    throw new Error('dsh-mobile: publicUrl invalid(需如 https://dsh.example.com/pair)')
  }
    ...
    handler: async (req, res) => {
      ...
      try {
        ...
        if (u.pathname === '/api/tokens' && req.method === 'GET') {
          ...
          // relay = 网关侧 connected(旧客户端仍持中转 WS,回退形态);'' = 离线。
