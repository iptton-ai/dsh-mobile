...
// ── 清洗/校验 ────────────────────────────────────────────────────────────
// settings 命名空间:上游 0.1.2-alpha 起 @deepseek-ai/dsh-settings 不再导出
// settingsNamespace branding 辅助(f4e49cc8f),宿主 settings.register 自带
// 同款 kebab-case 校验(/^[a-z][a-z0-9-]*$/),直接传字符串即可。
...
function createPairService({ getConfig, getLabel, onEvent, emit }) {
...
function createSignalService({ getConfig, emit, tickets }) {
  ...
  const state = () => ({
    connected: ws !== null,
    ticketValid: !tickets.expiringSoon(),
  ...
  // vstream 泵(frame/attachStream/dispatchFrame)为模块级共享函数:
  // 信令(DataChannel)与中转(WS)两服务同构共用,见文件上方。
  ...
  const startSession = async (msg) => {
    ...
    try {
      ...
      dc.onMessage((data) => {
        ...
        vsDispatchFrame(sess, (b) => dc.sendMessageBinary(b), buf, getConfig().localPort, emit)
      })
      dc.onClosed(() => {
        sess.dcOpen = false
  ...
  const connect = () => {
    ...
    let socket
    try {
      socket = new WebSocket(signalUrl() + '?ticket=' + encodeURIComponent(tickets.fresh()))
    } catch (e) {
...
function createRelayService({ getConfig, emit, tickets }) {
  let ctrl = null         // 控制腿 WebSocket
  let stopped = false
  let attempt = 0
  let reconnectTimer = null
  const pairs = new Map() // rsid → { ws, ready, streams, feeds, jti, device }
  ...
  const state = () => ({
    ...
    devices: [...pairs.values()]
      ...
      .map((p) => ({ jti: p.jti, device: p.device, streams: p.streams.size })),
  })
  ...
  const openDataLeg = async (rsid, msg) => {
    ...
    socket.onmessage = (ev) => {
      ...
      vsDispatchFrame(sess, (b) => socket.send(b), buf, getConfig().localPort, emit)
    }
    socket.onclose = () => {
  ...
  const connectCtrl = () => {
    ...
    socket.onclose = () => { if (ctrl === socket) { ctrl = null; scheduleReconnect() } }
    socket.onerror = () => { /* onclose 会跟 */ }
  }
...
export function apply(ctx, config = {}) {
  ...
  const configAdminKey = String(config?.adminKey ?? '').trim()
  ...
  const getConfig = () => ({
    ...runtime,
    adminKey: envAdminKey || settingsAdminKey || configAdminKey,
  })
  ctx.inject(['settings'], (sctx) => {
    labelScope = sctx.settings.register(LABEL_NS, SettingsSchema, {
      base: { label },
    })
    ...
    sctx.effect(() => labelScope.watch(applySettings))
  })
  ...
  const pair = createPairService({ getConfig, getLabel, onEvent: recordSecurityEvent, emit })
  // 票据管理器(信令/中转共用,统一 5min 刷新);信令 = P2P 备选,
  // 中转 = 默认数据链路(2026-09-10 还原:P2P 在办公网/蜂窝不可用)。
  const tickets = createTicketManager({ getConfig, emit })
  const signal = createSignalService({ getConfig, emit, tickets })
  const relay = createRelayService({ getConfig, emit, tickets })
  signal.start()
  relay.start()
  tickets.start()
      ...
      try {
        ...
        if (u.pathname === '/api/admin-key' && req.method === 'GET') {
          ...
          return sendJson(res, 200, {
            ...
            editable: labelScope !== null && envAdminKey.length < 16,
          })
        }
        ...
        // 移动接入状态(中转 = 默认链路 + 信令 = P2P 备选)。
        if (u.pathname === '/api/state' && req.method === 'GET') {
          return sendJson(res, 200, { ok: true, mode: 'relay', relay: relay.state(), signal: signal.state() })
        }
        ...
        if (u.pathname === '/api/tokens' && req.method === 'GET') {
          ...
          // 链路标注(按优先级):relay = 本机活跃中转配对;p2p = 本机活跃
          // DataChannel 会话;connected 近似(5min 内有管理面活动)= 'relay';
          // '' = 离线。
