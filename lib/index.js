...
function createPairService(options) {
  ...
  /** 当前可用传输清单(每次调用现算 —— adminKey/tenantKey 经 settings watch 可变)。 */
  const transports = () => {
    const list = []
    if (options.gateway && options.adminKey) list.push('cf')
    if (options.target || (options.adminUrl && options.tenantKey)) list.push('rust')
    return list
  }
  ...
  /** CF 形态:直连 Worker 管理面(ADMIN_KEY 信任根)。 */
  const cfFetch = (path, method, payload) => httpFetch(options.gateway, options.adminKey, path, method, payload)
  ...
  /** 单传输管理面调用;via = 'cf' | 'rust'。Rust 传输双模:配置了
   *  adminUrl+tenantKey(多租户 HTTPS 直连,网关侧按租户围栏)优先,
   *  否则走传统 ssh loopback 通道(运营者形态)。 */
  const adminCall = (via, path, method, payload) => {
    if (via === 'cf') return cfFetch(path, method, payload)
    if (options.adminUrl && options.tenantKey) {
      return httpFetch(options.adminUrl, options.tenantKey, path, method, payload)
    }
    return rustFetch(path, payload)
  }

  const unreachableHint = () => {
    const parts = []
    if (options.gateway) parts.push('CF 网关 ' + options.gateway + (options.adminKey ? '' : '(缺管理密钥)'))
    if (options.adminUrl && options.tenantKey) parts.push('Rust 管理面 ' + options.adminUrl)
    else if (options.target) parts.push('ssh ' + String(options.target) + ' 管理口 ' + String(options.adminPort))
    return parts.length > 0
      ? '网关管理面不可达:检查 ' + parts.join('、')
      : '无可用网关传输(未配 target / adminUrl+tenantKey / gateway+adminKey)'
  }

  /** 多宿主共享网关时的本机归属判定(tokens 清单过滤):Rust 按隧道端口、
   *  CF 按隧道主机名 —— 别家宿主配对的设备不再混进本机面板/在线角标。
   *  Rust 侧 port 为 null 的令牌(密码登录/默认上游,非配对签发)不展示:
   *  它不绑定任何宿主,多宿主下归属不明;单宿主部署建议关密码登录(CF 形态
   *  已移除)。CF 侧未配 cfHostname 时(靠 Worker 默认上游)保守不过滤。 */
  const belongsHere = (via, t) => {
    if (via === 'rust') {
      if (t.upstream_port === null || t.upstream_port === undefined) return false
      return Number(t.upstream_port) === Number(options.remotePort)
    }
    if (!options.cfHostname) return true
    return t.tunnel_host === options.cfHostname
  }
  ...
  return {
     ...
     *  多宿主/多租户过滤:只保留绑定本机(upstream_port/tunnel_host)的令牌 ——
     *  网关侧已按租户围栏(显式租户钥),这层再按宿主收敛,单运营者多机部署
     *  时每台 Mac 面板只见自己的设备。 */
    tokens: async () => {
      ...
      for (const { via, json } of lists) {
        if (Array.isArray(json)) {
          for (const t of json) {
            if (belongsHere(via, t)) merged.push({ ...t, via })
          }
        }
      }
    ...
    transports,
    /** Web 面(浏览器远程访问 dsh web)密码管理 —— 仅 Rust 网关有 Web 面。
     *  status:GET 查询启用状态;save:POST 明文密码(经 ssh base64 通道,
     *  网关侧 argon2 落库)/clear:true 关闭。CF 形态无此面,统一返回
     *  unsupported 提示。 */
    webPassword: async (action, password) => {
      if (!options.target) return { error: 'Web 面仅 Rust 网关支持(未配 ssh target)' }
      if (action === 'status') {
        const json = await rustFetch('/admin/web/password')
        return json ?? { error: unreachableHint() }
      }
      if (action === 'clear') {
        const json = await rustFetch('/admin/web/password', { clear: true })
        return json ?? { error: unreachableHint() }
      }
      if (typeof password !== 'string' || password.length < 8 || password.length > 128) {
        return { error: '密码需 8-128 字符' }
      }
      const json = await rustFetch('/admin/web/password', { password })
      return json ?? { error: unreachableHint() }
    },
  }
}
...
export function apply(ctx, config = {}) {
  ...
  const options = {
    ...
    // Rust 网关多租户(004):adminUrl = 公开管理面基址(如 https://gw.example.com,
    // nginx 反代的公开面 /admin/*);tenantKey = 运营者签发的租户密钥(settings 层
    // 管理,同 adminKey)。两者齐备时 Rust 管理调用走 HTTPS 直连(免 ssh;配对
    // claim/tokens/revoke 被网关按租户围栏),否则回落传统 ssh 通道(运营者形态)。
    adminUrl: (process.env.DSH_MOBILE_ADMIN_URL ?? config?.adminUrl ?? '').replace(/\/+$/, ''),
    tenantKey: process.env.DSH_MOBILE_TENANT_KEY ?? config?.tenantKey ?? '',
  ...
  if (options.adminUrl && !/^https:\/\/[a-zA-Z0-9.-]+(:\d+)?$/.test(options.adminUrl)) {
    throw new Error('dsh-mobile: invalid adminUrl ' + options.adminUrl + ' (expect https URL, no path)')
  }
  ...
  ctx.inject(['settings'], (sctx) => {
    ...
    options.tenantKey = sanitizeAdminKey(labelScope.get().tenantKey)
    sctx.effect(() => labelScope.watch((next) => {
      ...
      options.tenantKey = sanitizeAdminKey(next.tenantKey)
    }))
  })
    ...
    handler: async (req, res) => {
      ...
      try {
        ...
        if (u.pathname === '/api/tenant-key' && req.method === 'GET') {
          return sendJson(res, 200, {
            ok: true,
            configured: Boolean(options.tenantKey),
            masked: options.tenantKey ? options.tenantKey.slice(0, 4) + '…' + options.tenantKey.slice(-4) : '',
        ...
        if (u.pathname === '/api/tenant-key' && req.method === 'POST') {
          ...
          if (labelScope === null) {
            return sendJson(res, 503, { ok: false, error: 'settings service unavailable (dsh too old); set via DSH_MOBILE_TENANT_KEY env' })
          }
          ...
          try {
            await labelScope.update({ tenantKey: next })
          } catch (e) {
            return sendJson(res, 409, { ok: false, error: String(e && e.message ? e.message : e) })
          }
          return sendJson(res, 200, { ok: true, configured: next.length > 0 })
        ...
        if (u.pathname === '/api/mode' && req.method === 'GET') {
          return sendJson(res, 200, {
            ...
            // 多租户(Rust 形态 HTTPS 直连管理面;未配则走 ssh 运营者通道)。
            adminUrl: options.adminUrl,
            tenantKeyConfigured: Boolean(options.tenantKey),
          })
        }
