...
function createPairService(options) {
  ...
  // adminKey 可在 /pair 页随时填写(动态判定,不必重启)。
  ...
  const unreachableHint = () =>
    options.gateway && !options.adminKey
      ? '尚未配置 ADMIN_KEY:打开「移动接入」页填写管理密钥后重试'
      : cfMode()
        ? '网关不可达:检查 gateway ' + options.gateway + ' 与 ADMIN_KEY 是否正确'
...
<div class="card"><h2>管理密钥(CF 形态)</h2>
 <div style="display:flex;gap:8px">
  <input id="adminKey" type="password" autocomplete="off" spellcheck="false" placeholder="ADMIN_KEY(openssl rand -hex 32)"
   style="flex:1;min-width:0;padding:9px 12px;font-size:15px;border:1px solid #3a3a3c;border-radius:10px;background:#151516;color:#eee">
  <button id="setKey">保存</button></div>
 <span class="dim" id="keyhint">部署 Worker 时填的 ADMIN_KEY;保存在本机 dsh 设置(用户层),不进配置文件,保存即生效</span></div>
...
async function loadTunnel() {
  try {
    ...
    $('tunnel').innerHTML = t.mode === 'cloudflared'
      ? (t.up ? '<span class="ok">● 已连接</span>' : '<span class="bad">● 重连中(第 ' + t.attempts + ' 次)</span>')
        + ' &nbsp;cloudflared ' + esc(t.target) + ' ⇄ 本机 :' + t.localPort
      : (t.up ? '<span class="ok">● 已连接</span>' : '<span class="bad">● 重连中(第 ' + t.attempts + ' 次)</span>')
...
async function loadKey() {
  try {
    const k = await api('/api/admin-key');
    if (k.configured) $('keyhint').textContent = '已配置 ' + (k.masked || '') + '(留空保存 = 清除,回落配置/环境变量)';
    if (k.editable === false) $('adminKey').disabled = true;
  } catch (e) { $('adminKey').disabled = true; }
}
...
async function saveKey() {
  ...
  btn.disabled = true;
  try {
    const r = await api('/api/admin-key', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ adminKey: $('adminKey').value }) });
    btn.textContent = r.ok ? '已保存' : ('失败:' + (r.error || '?'));
    if (r.ok) { $('adminKey').value = ''; loadKey(); }
...
$('setKey').onclick = saveKey;
$('refresh').onclick = loadTokens;
loadTunnel(); loadLabel(); loadKey(); loadTokens();
...
export function apply(ctx, config = {}) {
  ...
  const options = {
    ...
    cfConfigPath: process.env.DSH_MOBILE_CF_CONFIG ?? config?.cfConfigPath ?? (homedir() + '/.cloudflared/config.yml'),
  ...
  // 用户层可在 /pair 页改,持久化;watch 让运行中的配对/二维码即时生效 ——
    ...
    handler: async (req, res) => {
      ...
      try {
        ...
        if (u.pathname === '/api/tunnel' && req.method === 'GET') {
          // CF 优先展示(对应配对 UI 的网关);Rust 隧道仍在跑,只是不在此行显示。
          const st = cfTunnel !== null ? cfTunnel.state() : (tunnel !== null ? tunnel.state() : { mode: 'none', up: false })
