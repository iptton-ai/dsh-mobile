...
// 配对完成/令牌吊销/Web 密码变更是「本地 foothold 远程化」的关键动作。本地
// 管理 API 的三重门只防浏览器,防不了同用户进程(头全可伪造)—— 无法在
// 进程内鉴权,只能把这些静默动作变显性:OS 通知即时提醒 + 面板横幅/侧栏
// 角标持久标记(数据源 /api/security-log,见 recordSecurityEvent)。
/** AppleScript 字符串安全化:双引号/反斜杠会破坏 osascript 字面量,替换之。 */
const asq = (s) => String(s ?? '').replace(/[\\"]/g, "'")

/** OS 通知(fail-silent):macOS osascript / Linux notify-send,其余平台跳过。
 * detached + 错误全吞:通知失败绝不影响业务路径。 */
...
function createPairService({ getGateways, getLabel, onEvent }) {
    ...
    while (myGen === generation && session !== null) {
      ...
      if (session.state === 'waiting') {
        ...
        if (j && j.status === 'confirmed') {
          ...
          if (onEvent) onEvent('paired', '设备「' + (session.device ?? '?') + '」(经 ' + nameOf(g) + ')')
          return
        }
  ...
  return {
    ...
    /** Web 面(浏览器远程访问 dsh web)密码管理 —— 按宿主端口(网关 005):
     *  请求一律带本机条目的 remotePort,网关按端口存取/路由;password/label
     *  动作附机器名(登录页下拉展示,网关 upsert hosts)。作用于第一个具备
     *  target 的 rust 条目。status:GET 查询;save:POST 明文密码(经 ssh
     *  base64 通道,网关侧 argon2 落库)/clear:true 关闭/label:仅刷宿主名。
     *  CF/纯租户形态无此面,统一 unsupported 提示。 */
    webPassword: async (action, password) => {
      ...
      const port = Number(g.remotePort)
      const label = String(getLabel() ?? '')
      if (action === 'status') {
        return (await call('/admin/web/password?port=' + String(port), 'GET')) ?? { error: unreachableHint(g) }
      }
      if (action === 'label') {
        return (await call('/admin/web/password', 'POST', { port, label })) ?? { error: unreachableHint(g) }
      }
      if (action === 'clear') {
        return (await call('/admin/web/password', 'POST', { port, clear: true })) ?? { error: unreachableHint(g) }
      }
      if (typeof password !== 'string' || password.length < 8 || password.length > 128) {
        return { error: '密码需 8-128 字符' }
      }
      return (await call('/admin/web/password', 'POST', { port, label, password })) ?? { error: unreachableHint(g) }
    },
  }
}
...
export function apply(ctx, config = {}) {
  ...
  // 宿主名推送到网关(登录页下拉展示名;见 pair 创建后的赋值)。先占位,
  // settings watch 早于 pair 服务存在即可安全调用。
  let pushHostLabel = () => {}
  ...
  const recordSecurityEvent = (kind, detail) => {
    ...
    const label = kind === 'paired' ? '新设备完成配对'
      : kind === 'revoked' ? '设备令牌被吊销'
      : 'Web 面密码已变更'
    emit('info', '安全事件:' + label + ' · ' + String(detail ?? ''))
    notifyUser('DSH 移动接入(dsh-mobile)', label + ':' + String(detail ?? '') + ' — 若非你本人操作,请打开「移动接入」面板处理')
  }

  const pair = createPairService({
    getGateways: () => gateways,
    getLabel: () => options.label,
    onEvent: recordSecurityEvent,
  })

  // 启动即推一次宿主名(网关 hosts upsert;管理通道独立于数据隧道,无需等
  // 隧道就绪)。webPassword 的目标行(第一个 rust+target)即推送目标。
  pushHostLabel = () => { void pair.webPassword('label').catch(() => {}) }
  pushHostLabel()
    ...
    handler: async (req, res) => {
      ...
      try {
        ...
        if (u.pathname === '/api/web-password' && req.method === 'POST') {
          ...
          if (body.clear === true) {
            const r = await pair.webPassword('clear')
            // 安全事件:Web 密码是网关侧持久化的远程访问通道,任何变更都通告。
            if (!(r !== null && r.error)) recordSecurityEvent('web-password', '密码已清除(浏览器远程访问关闭)')
            return sendJson(res, 200, r)
          }
          if (typeof body.password !== 'string') return sendJson(res, 400, { error: 'password required' })
          const r = await pair.webPassword('save', body.password)
          if (!(r !== null && r.error)) recordSecurityEvent('web-password', '密码已设置(浏览器将可远程访问本机)')
          return sendJson(res, 200, r)
        }
        ...
        // 安全事件日志(审计缓解):面板横幅/侧栏角标数据源。GET 只读;ack 确认
        // 后横幅与角标消失(ack 也是写操作,走全局 x-dsh-mobile 头门)。
